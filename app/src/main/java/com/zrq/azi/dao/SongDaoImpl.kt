...
class SongDaoImpl(
    ...
    private fun songToValues(song: Dj.ProgramsBean): ContentValues {
        ...
        values.put(FIELD_SONG_ID, song.mainSong.id)
    ...
    private fun cursorToSong(cursor: Cursor): Dj.ProgramsBean {
        ...
        song.mainSong = Dj.ProgramsBean.MainSongBean()
        song.mainSong.id = cursor.getLong(cursor.getColumnIndex(FIELD_SONG_ID)).toInt()
